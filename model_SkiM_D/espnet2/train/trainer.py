...
class Trainer:
    ...
    def train_one_epoch(
        ...
        iterator_stop = torch.tensor(0).to(("mps" if torch.backends.mps.is_available() else "cuda") if ngpu > 0 else "cpu")
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            batch = to_device(batch, ("mps" if torch.backends.mps.is_available() else "cuda") if ngpu > 0 else "cpu")
            if no_forward_run:
