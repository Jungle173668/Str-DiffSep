...
class SeparateSpeech:
    ...
    def __call__(
        ...
        if 'speech_ref1' in kwargs:
            speech_ref1 = kwargs['speech_ref1']
        ...
        if speech_ref1 is not None:
            speech_ref1 = speech_ref1.to(getattr(torch, self.dtype))
            speech_ref1 = to_device(speech_ref1, device=self.device)
        ...
        if self.segmenting and lengths[0] > self.segment_size * fs:
            ...
            if speech_ref1 is not None:
                speech_ref1, f_lens = self.enh_model.encoder(speech_ref1, lengths)

            if isinstance(self.enh_model, ESPnetDiffusionModel):
                feats = [self.enh_model.enhance(feats, N=self.diffn, streaming=self.streaming, speech_ref=speech_ref1, sampler=self.sampler, zero_his=self.zero_his)]
                # feats = [self.enh_model.enhance(feats, N=1000)]
            else:
                feats, _, _ = self.enh_model.separator(feats, f_lens, additional)
...
def inference(
    ...
    for i, (keys, batch) in tqdm.tqdm(enumerate(loader)):
        ...
        _lengths = batch['speech_mix_lengths'] 
        batch = {k: v for k, v in batch.items() if not k.endswith("_lengths")}

        waves = separate_speech(**batch, fs=fs)
        for spk, w in enumerate(waves):
            for b in range(_lengths.shape[0]):
                writers[spk][keys[b]] = fs, w[b][0:_lengths[b]]
