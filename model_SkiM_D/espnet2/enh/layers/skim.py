...
from espnet2.enh.layers.ncsnpp_utils import layers, layerspp
...
class MemLSTM(nn.Module):
    ...
    def __init__(
        ...
        if use_t_emb:
            self.temblayer =  torch.nn.Sequential(
            layerspp.GaussianFourierProjection(
                    embedding_size=self.hidden_size, scale=16,
                ),
            torch.nn.Linear(self.hidden_size*2, self.hidden_size * (2 if bidirectional else 1)),
            nn.SiLU()
            )
    ...
    def forward_one_step(self, hc, state, t=None):
        if self.use_t_emb:
            temb = self.temblayer(torch.log(t))
            pass
        ...
        if self.mem_type == "id":
            ...
            h = h.view(B, d, H).transpose(1, 0).contiguous()
            c = c.view(B, d, H).transpose(1, 0).contiguous()
            hc = (h, c)
...
class SegLSTM(nn.Module):
    ...
    def __init__(
        self, input_size, hidden_size, dropout=0.0, bidirectional=False, norm_type="cLN", temb_dim=None, with_conformer=False,
        c_ffn=1024,
        c_att_head=4,
        c_kernel=7,
    ):
        ...
        if with_conformer:
            self.conformer_layer = ConformerLayer(
                ...
                ffn_dim=c_ffn,
                num_attention_heads=c_att_head,
                depthwise_conv_kernel_size=c_kernel,
                dropout=dropout,
            )
    ...
    def forward(self, input, hc):
        ...
        if self.with_conformer:
            # T, B, H
            input = self.conformer_layer(input.permute(1, 0, 2), None).permute(1, 0 , 2)
...
class SegBlock(SegLSTM):
    def __init__(self, input_size, hidden_size, dropout=0.0, bidirectional=False, norm_type="cLN", temb_dim=None, ii=-1, with_dcunet=False):
        super().__init__(
            input_size*2, hidden_size, dropout, bidirectional, norm_type, temb_dim
        )

        self.input_channel = 2 if ii == 0 else 1
        self.with_dcunet = with_dcunet

        if with_dcunet:
            self.dcunet = DCUNet(dcunet_architecture='DilDCUNet-v2', input_channel=self.input_channel)
        elif self.input_channel == 2:
            self.input_fc = torch.nn.Conv2d(in_channels=4, out_channels=2, kernel_size=1)
            


    def forward(self, input_complex, hc, t=None):
        # input shape: B, T, H
        B, T, H = input_complex.shape
        input_complex = input_complex.view(B, T, self.input_channel, -1)
        B, T, C, F = input_complex.shape
        input_complex = input_complex.permute(0, 2, 3, 1)

        if self.with_dcunet:
            input_complex = self.dcunet(input_complex, t) # B, C, F, T
        elif self.input_channel == 2:
            tmp = torch.cat([input_complex.real, input_complex.imag], dim=1)
            tmp = self.input_fc(tmp)
            input_complex = tmp[:, [0], :, :] + tmp[:, [1], :, :]*1j

        # return input_complex, hc
        ...
        ).permute(0, 3, 1, 2).view(B, T, C * 2 * F)

        if hc is None:
            # In fist input SkiM block, h and c are not available
            d = self.num_direction
            h = torch.zeros(d, B, self.hidden_size, dtype=input_lstm.dtype).to(input_lstm.device)
            c = torch.zeros(d, B, self.hidden_size, dtype=input_lstm.dtype).to(input_lstm.device)
        else:
            h, c = hc

        output, (h, c) = self.lstm(input_lstm, (h, c))
        output = self.dropout(output)
        output = self.proj(output.contiguous().view(-1, output.shape[2])).view(
            input_lstm.shape
        )
        output = input_lstm + self.norm(output)
        # output = input_lstm 

        output_lstm = output.view(B, T, 2, C, F)
        output_lstm = output_lstm[:, :, 0, :, :] + output_lstm[:, :, 1, :, :] * 1j
        output_lstm = output_lstm.permute(0, 2, 3, 1)

        output = output_lstm # B, C, F, T
        output = output.permute(0,3, 1, 2).view(B, T, F).contiguous()
...
class SkiM(nn.Module):
    ...
    def _padfeature(self, input):
        ...
        if rest > 0:
            input = torch.nn.functional.pad(input, (0, 0, 0, rest), mode='reflect')
        return input, rest
...
class SkiMComplex(SkiM):
        ...
        for i in range(num_blocks):
            self.seg_lstms.append(
                SegBlock(
                    ...
                    bidirectional=bidirectional,
                    norm_type=norm_type,
                    temb_dim=hidden_size*2,
                    ii=i,
                    with_dcunet=with_dcunet,
                )
            )
        self.output_fc = None
    ...
    def forward(self, input, t):
        ...
        t = torch.cat([t[:, None]] * S, dim=-1).view(-1)

        output = input.view(B * S, K, D).contiguous()  # BS, K, D
        hc = None
        for i in range(self.num_blocks):
            output, hc = self.seg_lstms[i](output, hc, t)  # BS, K, D
            if self.mem_type and i < self.num_blocks - 1:
                hc = self.mem_lstms[i](hc, S)
                pass

        if self.seg_overlap:
            output = output.view(B, S, K, F).permute(0, 3, 2, 1)  # B, D, K, S
            output = merge_feature(output, rest).unsqueeze(1)  # B, D, T

        else:
            output = output.view(B, S * K, F)[:, :T, :]  # B, T, D
