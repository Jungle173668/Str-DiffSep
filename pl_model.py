...
class DiffSepModel(pl.LightningModule):
    ...
    # def forward(self, xt, time, mix, skim_inference=None):
    def forward(self, xt, time, mix):
        # implement inference here
        # return self.score_model(xt, time, mix, skim_inference)
        return self.score_model(xt, time, mix)

    def forward_streaming(self, xt, time, mix, states, skim_inference=None):
        #Streaming inference
        return self.score_model.forward_streaming(xt, time, mix, states, skim_inference=skim_inference)
    
    def forward_equal(self, xt, time_cond, mix, equal_score): 
        return self.score_model.forward_equal(xt, time_cond, mix, equal_score)
    ...
    def compute_score_loss(self, mix, target):
        ...
        # true score: L^-1 * z
        # true_score = - self.sde.mult_std_inv(L, z)
    ...
    def compute_real_score(self, mix, target):
        x_t, time, L, z = self.sample_prior(mix, target)
    ...
    def validation_step(self, batch, batch_idx, dataset_i=0):
        # original mix and target are used for recording.
        original_mix, original_target = batch
        ...
        if self.trainer.testing or self.n_batches_est_done < self.valid_max_sep_batches:
            ...
            est, *_ = self.separate(original_mix)

            for name, loss in self.val_losses.items():
                self.log(name, loss(est, original_target), on_epoch=True, sync_dist=True)
    ...
    def get_pc_sampler_multidiff(
        ...
        **kwargs,
        ...
        # compute real score using discriminative model
        

        return sdes.get_pc_sampler_multidiff(
                predictor_name,
                corrector_name,
                sde=sde,
                score_fn=self.forward_streaming,
                backbone =self.score_model.backbone,
                true_score_fn=self.compute_real_score,
                skim_inference=skim_inference,
                y=y,
                **kwargs,
            )
    ...
    def get_pc_sampler_multidiff_equal(
        ...
        **kwargs,
        ...
        return sdes.get_pc_sampler_multidiff_equal(
                predictor_name,
                corrector_name,
                sde=sde,
                score_fn_equal=self.forward_equal,
                score_fn_full=self.forward,
                score_stream=self.forward_streaming,
                backbone=self.score_model.backbone,
                # true_score_fn=self.compute_real_score,
                skim_inference=skim_inference,
                y=y,
