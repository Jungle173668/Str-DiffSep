...
class WSJ0_mix(torch.utils.data.Dataset):
    def __init__(
        ...
        self.path = (
            self.base_folder
            / f"Libri2Mix/wav{self.fs // 1000}k/{cut}/{split_map[split]}"
        )
        ...
        # Apply subset ratio first if specified
        if subset_ratio is not None:
            n_subset = int(len(self.file_list) * subset_ratio)
            # Randomly sample a subset of files
            random.seed(42)  # For reproducibility
            self.file_list = random.sample(self.file_list, n_subset)
            self.file_list = sorted(self.file_list)  # Sort again for consistency
            log.info(f"Randomly using {subset_ratio*100:.1f}% of {split} dataset: {n_subset}/{len(sorted(os.listdir(self.path_mix)))} samples")
...
class WSJ0_mix_Module(pl.LightningDataModule):
    ...
    def setup(self, *args, **kwargs):
        for split in self.cfg.datamodule:
            if 'libri' in split:
                # For LibriMix, we use a different dataset class
                # continue
                self.datasets[split] = instantiate(self.cfg.datamodule[split].dataset)
        
        # Print the datasets for each split (skip libri datasets)
        for split in self.cfg.datamodule:
            if 'libri' in split:
                continue
                print(split, self.datasets[split])
